...
source(here::here("code/common_basis.R"))
```
...
vd17_identical_all <- t %>%
  inner_join(t, join_by(title, normalized_year, places_of_publication, printer_gnds, extents, form_factors, record_number < record_number)) %>%
  select(record_number.x, record_number.y, title, normalized_year, places_of_publication, printer_gnds, extents, form_factors) %>%
  compute_a()
...
```{r}
evaluation_1 <- read_sheet(ss = "1YeGLSHzLyg1KlyYdh4wgqM-R5EHYl_fH6udInEj9CQQ", sheet = "mapping evaluation 1")
```

```{r}
evaluation_1 %>%
  select(vd17_id.x, vd17_id.y, status = `same printing (variant)`) %>%
  copy_to_a(con) %>%
  inner_join(vd17_id_a %>% rename(record_number = record_number, vd17_id.x = vd17_id)) %>%
  inner_join(vd17_id_a %>% rename(source_record_number = record_number, vd17_id.y = vd17_id)) %>%
  left_join(vd17_variant_mapping_a %>% union_all(vd17_variant_mapping_a %>% distinct(record_number) %>% mutate(source_record_number = record_number) %>% mutate(mapped = T), join_by(source_record_number)))
```
