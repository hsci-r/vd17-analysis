...
vd17_normalized_years_c <- vd17_normalized_years_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_normalized_years_c"), overwrite = TRUE)
...
vd17_genres_c <- vd17_genres_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_genres_c"), overwrite = TRUE)
...
vd17_normalized_locs_a <- vd17_a %>%
  ...
  copy_to_a(con, name = dbplyr::in_schema("vd17", "vd17_normalized_locs_a"), temporary = FALSE, overwrite = TRUE, indexes = list(c("record_number", "field_number")))

vd17_normalized_locs_c <- vd17_normalized_locs_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_normalized_locs_c"), overwrite = TRUE)
...
vd17_normalized_langs_c <- vd17_normalized_langs_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_normalized_langs_c"), overwrite = TRUE)
...
vd17_titles_c <- vd17_titles_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_titles_c"), overwrite = TRUE)
...
vd17_wide_c <- vd17_wide_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_wide_c"), overwrite = TRUE)
```
...
fbs_metadata_c <- fbs_metadata_a %>%
  compute_c(name = "fbs_metadata_c", overwrite = TRUE)
...
fbs_gnds_c <- fbs_gnds_a %>%
  compute_c(name = "fbs_gnds_c", overwrite = TRUE)
...
fbs_links_c <- fbs_links_a %>%
  compute_c(name = "fbs_links_c", overwrite = TRUE)
...
vd17_auth_id_c <- vd17_auth_id_a %>%
  compute_c(name = "vd17_auth_id_c", overwrite = TRUE)
...
vd17_auth_gender_c <- vd17_auth_gender_a %>%
  compute_c(name = "vd17_auth_gender_c", overwrite = TRUE)
```
