...
# The below plots depicts the genre frequency of FBS translations for Society-related genres.
```{r}
...
fbs_translation_cat1 <- fbs_rescue %>%
  ...
  inner_join(vd17_normalized_langs_local%>%select(record_number, publication_language, intermediary_language, original_language), by = c("record_number"))%>%
  ...
  select(record_number,normalized_year,vd17_id,combined_all_titles,member_number,first_name,last_name,role2,field_code,original_language,intermediary_language,publication_language,notes,genre_full,combined_all_genres,rescue_value,rescue_field,resuce_additional_field,corporate_subfield_code_a,corporate_subfield_code_7,corporate_subfield_code_9,corporate_subfield_code_B,name_corporate,gnd_corporate,ppn_corporate,role_corporate,multipart)%>%
...
```
```{r}
genres_number_fbs_cat1 <- fbs_translation_cat1%>%
...
genres_number_fbs_cat1%>%
  ...
  xlab("Year") + ylab("FBS-translations-Society-related genres")+
...
genres_number_fbs_cat1%>%
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```
# Genre frequency of FBS translations by year per author for Society-related genres.
```{r}
genres_members_fbs_cat1 <- fbs_translation_cat1%>%
...
# The below plots depicts the genre frequency of FBS publications for Society-unrelated genres.
```{r}
genre_cat2 <- vd17_genres_local%>%
  filter(grepl(paste(c(genre_list2_a,genre_list2_b,genre_list2_c,genre_list2_d,genre_list2_e,genre_list2_f,genre_list2_g),collapse="|"), genre_full))%>%
  select(record_number,genre_full)

fbs_translation_cat2 <- fbs_rescue %>%
  left_join(vd17_normalized_years_local, by = c("record_number")) %>%
  filter(normalized_year >= 1600, normalized_year <= 1700) %>%
  filter(nchar(normalized_year)==4)%>%
  inner_join(genre_cat2,by = c("record_number"))%>%
  left_join(vd17_titles_local %>% select(record_number,combined_all_titles),by = c("record_number"))%>%
  inner_join(vd17_normalized_langs_local%>%select(record_number, publication_language, intermediary_language, original_language), by = c("record_number"))%>%
  left_join(vd17_genres_combined, by=c("record_number"))%>%
  left_join(translations_notes, by = c("record_number"))%>%
...
```{r}
genres_number_fbs_cat2 <- fbs_translation_cat2%>%
  select(vd17_id,record_number,normalized_year,genre_full)%>%
  distinct(vd17_id,record_number,normalized_year,genre_full)%>%
  group_by(normalized_year) %>%
  count(genre_full)%>%
...
genres_number_fbs_cat2%>%
  ggplot(aes(x=normalized_year,y=n,fill=genre_category))+
  geom_col(show.legend = FALSE)+
  facet_wrap(~genre_category, scales="free")+
  xlab("Year") + ylab("FBS-translations-Society-unrelated genres")+
...
genres_number_fbs_cat2%>%
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```
# Genre frequency of FBS translations by year per author for Society-unrelated genres.
```{r}
genres_members_fbs_cat2 <- fbs_translation_cat2%>%
  select(vd17_id,record_number,normalized_year,member_number,genre_full)%>%
  distinct(vd17_id,record_number,normalized_year,member_number,genre_full)%>%
  group_by(normalized_year,member_number) %>%
  count(genre_full)%>%
...
# The below plots depicts the genre frequency of FBS publications for neutral/NA genres.
```{r}
...
fbs_translation_cat3 <- fbs_rescue %>%
  left_join(vd17_normalized_years_local, by = c("record_number")) %>%
  filter(normalized_year >= 1600, normalized_year <= 1700) %>%
  filter(nchar(normalized_year)==4)%>%
...
```{r}
genres_number_fbs_cat3 <- fbs_translation_cat3%>%
  select(vd17_id,record_number,normalized_year,genre_full)%>%
  distinct(vd17_id,record_number,normalized_year,genre_full)%>%
  group_by(normalized_year) %>%
  count(genre_full)%>%
...
genres_number_fbs_cat3%>%
  ...
  geom_col(show.legend = FALSE)+
  facet_wrap(~genre_category, scales="free")+
  xlab("Year") + ylab("FBS-translations-Neutral/NA genres")+
  scale_x_continuous(breaks = seq(1000, 2000, by = 2))+
  theme_hsci_discrete()+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
...
# Genre frequency of FBS translations by year per author for neutral/NA genres.
```{r}
genres_members_fbs_cat3 <- fbs_translation_cat3%>%
  select(vd17_id,record_number,normalized_year,member_number,genre_full)%>%
  distinct(vd17_id,record_number,normalized_year,member_number,genre_full)%>%
  group_by(normalized_year,member_number) %>%
  count(genre_full)%>%
...
genre_fbs_cat1_link <- fbs_translation_cat1%>%
...
genre_fbs_cat2_link <- fbs_translation_cat2%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
my_data_frames_2 <- list(genre_fbs_cat2_link,genres_number_fbs_cat2,genres_members_fbs_cat2)

genre_fbs_cat3_link <- fbs_translation_cat3%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
my_data_frames_3 <- list(genre_fbs_cat3_link,genres_number_fbs_cat3,genres_members_fbs_cat3)

fbs_translations_genre_society_related<- gs4_create(
  "fbs_translations_genre_society_related",
  sheets = my_data_frames_1)

fbs_translations_genre_society_unrelated<- gs4_create(
  "fbs_translations_genre_society_unrelated",
  sheets = my_data_frames_2)

fbs_translations_genre_neutral_NA<- gs4_create(
  "fbs_translations_genre_neutral_NA",
  sheets = my_data_frames_3)

fbs_publications_genre_society_related
fbs_publications_genre_society_unrelated
