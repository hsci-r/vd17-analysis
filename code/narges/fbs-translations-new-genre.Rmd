...
vd17_translations_defined_fbs <- vd17_translations_defined%>%
  ...
  inner_join(fbs_links_of_interest_matches,by=c("record_number","vd17_id"))%>%
...
fbs_translation_cat1 <- fbs_rescue %>%
  ...
  select(record_number,normalized_year,vd17_id,combined_all_titles,member_number,first_name,last_name,role2,field_code,original_language,intermediary_language,publication_language,translations_notes,corporate_body_genre,genre_full,combined_all_genres,rescue_value,rescue_field,resuce_additional_field,field_code_corporate_exceptions,corporate_subfield_code_a,corporate_subfield_code_7,corporate_subfield_code_9,corporate_subfield_code_B,name_corporate,gnd_corporate,ppn_corporate,role_corporate,multipart)%>%
  mutate(genre_category = case_when(
    ...
    (!(grepl(paste(c(genre_list1_a,genre_list1_b,genre_list1_c,genre_list1_d,genre_list1_e,genre_list1_f,genre_list1_g,genre_list1_h,genre_list1_i),collapse="|"), genre_full))) ~ "corporate genre"
  ))%>%
  distinct()
...
fbs_translation_cat2 <- fbs_rescue %>%
  ...
  left_join(vd17_parts_of_multipart_works,by=c("record_number"))%>%
  left_join(vd17_corporate, by=c("record_number"))%>%
  select(record_number,normalized_year,vd17_id,combined_all_titles,member_number,first_name,last_name,role2,field_code,original_language,intermediary_language,publication_language,translations_notes,genre_full,combined_all_genres,rescue_value,rescue_field,resuce_additional_field,corporate_subfield_code_a,corporate_subfield_code_7,corporate_subfield_code_9,corporate_subfield_code_B,name_corporate,gnd_corporate,ppn_corporate,role_corporate,multipart)%>%
  mutate(genre_category = case_when(
...
genres_number_fbs_cat3%>%
  ...
  geom_col(show.legend = FALSE)+
  facet_wrap(~genre_category, scales="free")+
  xlab("Year") + ylab("FBS-translations-Neutral")+
  scale_x_continuous(breaks = seq(1000, 2000, by = 2))+
  theme_hsci_discrete()+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
