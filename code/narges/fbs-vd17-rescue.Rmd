...
genres_vd17_cat1 <- vd17_a_rescue %>%
  left_join(vd17_normalized_years_local, by = c("record_number")) %>%
  filter(normalized_year >= 1600, normalized_year <= 1700) %>%
  filter(nchar(normalized_year)==4)%>%
  inner_join(genre_cat1,by = c("record_number"))%>%
...
genres_vd17_cat2 <- vd17_a_rescue %>%
  left_join(vd17_normalized_years_local, by = c("record_number")) %>%
  filter(normalized_year >= 1600, normalized_year <= 1700) %>%
  filter(nchar(normalized_year)==4)%>%
  inner_join(genre_cat2,by = c("record_number"))%>%
...
genres_vd17_rescue <- vd17_a_rescue %>%
  left_join(vd17_genres_local, by = c("record_number"))%>%
  select(record_number,genre_full)

vd17_rescue_without_genre <- genres_vd17_rescue%>%
  filter(is.na(genre_full))

genre_na_cat3 <- rbind(vd17_rescue_without_genre,vd17_genres_local_cat3)

genres_vd17_cat3 <- vd17_a_rescue %>%
  left_join(vd17_normalized_years_local, by = c("record_number")) %>%
  filter(normalized_year >= 1600, normalized_year <= 1700) %>%
  filter(nchar(normalized_year)==4)%>%
  inner_join(genre_na_cat3,by = c("record_number"))%>%
...
genre_fbs_cat2_link <- genres_fbs_cat2%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
genre_fbs_cat3_link <- genres_fbs_cat3%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
my_data_frames_1 <- list(genre_fbs_cat1_link,genre_fbs_cat2_link,genre_fbs_cat3_link)

genres_vd17_cat1_link <- genres_vd17_cat1%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
genres_vd17_cat2_link <- genres_vd17_cat2%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
genres_vd17_cat3_link <- genres_vd17_cat3%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
my_data_frames_2 <- list(genres_vd17_cat1_link,genres_vd17_cat2_link,genres_vd17_cat3_link)

fbs_publications_genres<- gs4_create(
  "fbs_rescue_publications_genre",
  sheets = my_data_frames_1)

vd17_publications_genre<- gs4_create(
  "vd17_rescue_publications_genre",
  sheets = my_data_frames_2)

fbs_publications_genres

vd17_publications_genre
