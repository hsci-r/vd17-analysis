...
fbs_rescue <- fbs_links_of_interest_matches %>%
  merge(vd17_a_rescue, by=c("record_number"),all.x=TRUE)
```
...
vd17_corporate_name_gnd <- merge(vd17_corporate_authors_name,vd17_corporate_authors_gnd, by=c("record_number"),all.x=TRUE)%>%
  distinct()
vd17_corporate <- merge(vd17_corporate_name_gnd,vd17_corporate_authors_ppn, by=c("record_number"),all.x=TRUE)%>%
  distinct()
