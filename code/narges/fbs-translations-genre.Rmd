...
```{r}
genre_list2_a <- c("Educational","Schul- und Hochschulschriften","Hochschulschrift","Dissertation","Dissertationensammlung","Matrikel","Universitätsprogramm","Vorlesung","Vorlesungsverzeichnis","Jugendsachbuch","Rechenbuch","Schulbuch","Fibel","Lesebuch","Schulprogramm")
genre_list2_b <- c("Governmental","Amtsdruckschrift","Edikt","Gesetz","Gesetzesammlung","Kapitulation","Mandat","Privileg","Richtlinie","Verordnung","Vertrag")
genre_list2_c <- c("Judicial","Legal","Entscheidungssammlung","Fallsammlung","Konsiliensammlung","Konsilium","Satzung")
genre_list2_d <- c("Religious","Theological","Agende","Beichtspiegel","Bibel","Brevier","Gebet","Katechismus","Missale","Psalter","Rituale","Other Governmental and Institutional","Formularsammlung","Schreibmeisterbuch","Regesten","Urkundenbuch")
...
genre_list2_f <- c("War Related","Festungsbau","Kriegskunde")
genre_list2_g <- c("General Works","Bericht","Brief","Briefsammlung","Regelsammlung","Statistik","Streitschrift")
```
...
fbs_translation_cat1 <- fbs_rescue %>%
  ...
  left_join(vd17_titles_local %>% select(record_number,title_additional_field))%>%
  left_join(vd17_normalized_langs_local, by = c("record_number"))%>%
  filter(!is.na(original_language))%>%
  inner_join(vd17_genres_local_cat1, by = c("record_number"))%>%
...
genres_translations_fbs_cat1%>%
  ...
  xlab("Year") + ylab("Translations_FBS-Society-related genres")+
...
```{r}
vd17_genres_local_cat2 <- vd17_genres_local%>%
  filter(genre %in% c(genre_list2_a,genre_list2_b,genre_list2_c,genre_list2_d,genre_list2_e,genre_list2_f,genre_list2_g))%>%
  select(record_number,genre,genre_full)
...
fbs_translation_cat2 <- fbs_rescue %>%
  ...
  left_join(vd17_normalized_langs_local, by = c("record_number"))%>%
  filter(!is.na(original_language))%>%
  inner_join(vd17_genres_local_cat2, by = c("record_number"))%>%
  left_join(vd17_genres_combined, by=c("record_number"))%>%
  left_join(translations_notes, by = c("record_number"))%>%
  left_join(vd17_parts_of_multipart_works,by=c("record_number"))%>%
  left_join(vd17_corporate, by=c("record_number"))%>%
...
genres_translations_fbs_cat2%>%
  ...
  geom_col(show.legend = FALSE)+
  facet_wrap(~genre_category, scales="free_y")+
  xlab("Year") + ylab("Translations_FBS-Society-unrelated genres")+
  scale_x_continuous(breaks = seq(1000, 2000, by = 5))+
  theme_hsci_discrete()+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
...
fbs_translation_cat3 <- fbs_rescue %>%
  ...
  left_join(vd17_normalized_langs_local, by = c("record_number"))%>%
  filter(!is.na(original_language))%>%
  inner_join(vd17_genres_local_cat3, by = c("record_number"))%>%
  left_join(vd17_genres_combined, by=c("record_number"))%>%
  left_join(translations_notes, by = c("record_number"))%>%
  left_join(vd17_parts_of_multipart_works,by=c("record_number"))%>%
  left_join(vd17_corporate, by=c("record_number"))%>%
...
genres_translations_fbs_cat3%>%
  ...
  geom_col(show.legend = FALSE)+
  facet_wrap(~genre_category, scales="free_y")+
  xlab("Year") + ylab("Translations_FBS-neutral genres")+
  scale_x_continuous(breaks = seq(1000, 2000, by = 5))+
  theme_hsci_discrete()+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
...
my_data_frames <- list(fbs_translation_cat1,genres_translations_fbs_cat1,genres_members_translations_cat1,fbs_translation_cat2,genres_translations_fbs_cat2,genres_members_translations_cat2,fbs_translation_cat3,genres_translations_fbs_cat3,genres_members_translations_cat3)


fbs_translations_genre<- gs4_create(
  "fbs_translations_genre",
