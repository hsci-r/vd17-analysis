...
fbs_translation_cat1 <- fbs_links_of_interest_matches %>%
  ...
  inner_join(vd17_genres_local, by = c("record_number"))%>%
...
genres_translations_fbs_cat1 <- fbs_translation_cat1%>%
  ...
  na.omit(fbs_translation_cat1$genre)%>%
...
genres_translations_fbs_cat2 <- fbs_translation_cat2%>%
  select(vd17_id,record_number,normalized_year,genre)%>%
  distinct(vd17_id,record_number,normalized_year,genre)%>%
  na.omit(fbs_translation_cat2$genre)%>%
  group_by(normalized_year) %>%
  count(genre)%>%
  arrange(desc(n))%>%
  mutate(genre_category = case_when(
...
genres_translations_fbs_cat3 <- fbs_translation_cat3%>%
  select(vd17_id,record_number,normalized_year,genre)%>%
  distinct(vd17_id,record_number,normalized_year,genre)%>%
  na.omit(fbs_translation_cat3$genre)%>%
  group_by(normalized_year) %>%
  count(genre)%>%
  arrange(desc(n))%>%
  mutate(genre_category = case_when(
...
```
```{r}
translations_fbs_society_related <- gs4_create(
  "translations_fbs_society_related",
...
genre_translations_fbs_society_related <- gs4_create(
  "genres_translations_fbs_society_related",
  sheets =genres_translations_fbs_cat1)
genre_translations_fbs_society_related

genres_members_translations_society_related <- gs4_create(
  "genres_members_translations_society_related",
  sheets =genres_members_translations_cat1)
genres_members_translations_society_related 
```

```{r}
fbs_translation_society_unrelated_genres <- gs4_create(
  "genres_translations_fbs_society_unrelated",
  sheets = fbs_translation_cat2%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")'))))
fbs_translation_society_unrelated_genres

society_unrelated_genres_translations_fbs <- gs4_create(
  "genres_translations_fbs_society_unrelated",
  sheets =genres_translations_fbs_cat2)
society_unrelated_genres_translations_fbs


genres_members_translations_society_unrelated <- gs4_create(
  "genres_members_translations_society_unrelated",
...
```{r}
fbs_translation_neutral <- gs4_create(
  "genres_translations_fbs_neutral",
  sheets = fbs_translation_cat3%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")'))))
fbs_translation_neutral

genre_translations_fbs_neutral <- gs4_create(
  "genres_translations_fbs_neutral",
  sheets =genres_translations_fbs_cat3)
genre_translations_fbs_neutral


genre_members_translations_neutral <- gs4_create(
  "genres_members_translations_neutral",
  sheets =genres_members_translations_cat3)
genre_members_translations_neutral 
```
