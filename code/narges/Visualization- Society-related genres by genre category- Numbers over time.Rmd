...
plot1 <- core_society_genre %>%
  ...
  arrange(desc(n)) %>%
  distinct() %>%
  ggplot(aes(x = normalized_year, y = n, fill = genre_category, colour = genre_category)) +
  geom_point(size = 3) +
...
p1 <- core_society_frequency %>%
  ggplot(aes(x = normalized_year, y = percentage, fill = genre_category, colour = genre_category)) +
  geom_point(size = 3) +
