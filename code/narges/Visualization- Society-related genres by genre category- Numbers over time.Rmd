...
vd17_fbs <- vd17_normalized_langs_local%>%
  ...
  filter(normalized_year >= 1600, normalized_year <= 1700) %>%
  filter(nchar(normalized_year)==4)%>%
  left_join(fbs_links_of_interest_matches,by=c('record_number')) %>%
  select ('normalized_year','record_number','vd17_id','member_number','first_name', 'last_name','role2','field_code')%>%
