...
```
# These are rescue additional fields which Thea mentioned.
```{r}
...
# Joining vd17_id with fbs_links_of_interest (including all matches)
```{r}
...
```
# Creating society-subset including all-matches
```{r}
...
```
# This is related to society-related genres (as Thea asked we remove the records which are in the spreadsheet only because of rescue words)
```{r}
fbs_rescue_1 <- fbs_links_of_interest_matches %>%
  left_join(vd17_a_rescue, by=c("record_number","vd17_id"))%>%
  distinct()
```

# This is related to neutral/NA genres which we have all records with rescue words
```{r}
...
# Translations checking for society-related and corporate bodies for vd17_ids as exceptions which is mentioned in Genres v4 doc.
...
# This spreadsheet includes FBS publications with Society-related genres.
```{r}
...
genres_fbs_cat1 <- fbs_rescue_1 %>%
  ...
  distinct()
```
# These spreadsheet and plots show the frequency of society-related genres per year.
...
genres_number_fbs_cat1%>%
  ...
  xlab("Year") + ylab("FBS-Society-related genres")+
...
# The table for Neutral genres
```{r}

vd17_genres_combined_edited <- vd17_genres_combined%>%
  filter(grepl(paste(c(genre_list3_a,genre_list3_b,genre_list3_c),collapse="|"), combined_all_genres))%>%
  filter(!(grepl(paste(c(genre_list1_a,genre_list1_b,genre_list1_c,genre_list1_d,genre_list1_e,genre_list1_f,genre_list1_g,genre_list1_h,genre_list1_i),collapse="|"), combined_all_genres)))%>%
  filter(!(grepl(paste(c(genre_list2),collapse="|"), combined_all_genres)))%>%
  mutate(genre_full=combined_all_genres)%>%
  select(record_number,genre_full)

```

# # This spreadsheet includes FBS publications with Neutral/NA genres.
```{r}
...
genre_na_cat3 <- rbind(fbs_without_genre,vd17_genres_combined_edited)
...
genres_fbs_cat3 <- fbs_rescue %>%
  ...
  left_join(vd17_parts_of_multipart_works,by=c("record_number"))%>%
  left_join(vd17_corporate, by=c("record_number"))%>%
  select(record_number,normalized_year,vd17_id,combined_all_titles,member_number,first_name,last_name,role2,field_code,original_language,publication_language,genre_full,rescue_value,rescue_field,resuce_additional_field,corporate_subfield_code_a,corporate_subfield_code_7,corporate_subfield_code_9,corporate_subfield_code_B,name_corporate,gnd_corporate,ppn_corporate,role_corporate,multipart)%>%
  mutate(genre_category = case_when(
  ...
  distinct()
```
# These spreadsheet and plots show the frequency of Neutral/NA genres per year.
```{r}
...
genres_number_fbs_cat3%>%
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```
# Genre frequency of FBS publications by year per member for Neutral/NA genres.
```{r}
...
```
# Creating the google spreadsheet for society-related spreadsheets
```{r}
genre_fbs_cat1_link <- genres_fbs_cat1%>%
...
# Creating the google spreadsheet for neutral/NA spreadsheets
```{r}
genre_fbs_cat3_link <- genres_fbs_cat3%>%
     mutate(vd17_id=gs4_formula(str_c('=HYPERLINK("https://kxp.k10plus.de/DB=1.28/CMD?ACT=SRCHA&IKT=8079&TRM=%27',vd17_id,'%27","',vd17_id,'")')))
