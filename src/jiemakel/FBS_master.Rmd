...
fbs_auto_records_a %>%
  ...
  left_join(
    fbs_records_a %>%
      group_by(vd17_id) %>%
      summarise(db_status = case_when(
        any(set == "Active member substantive role and translation into German") ~ "Active member substantive role and translation into German",
        any(set == "Active member substantive role and society purpose related") ~ "Active member substantive role and society purpose related",
        any(set == "Active member substantive role") ~ "Active member substantive role",
        any(set == "Active member linked") ~ "Active member linked",
        any(set == "Member substantive role and translation into German") ~ "Non-active member substantive role and translation into German",
        any(set == "Member substantive role and society purpose related") ~ "Non-active member substantive role and society purpose related",
        any(set == "Member substantive role") ~ "Non-active member substantive role",
        any(set == "Member linked") ~ "Non-active member linked"
      )),
    join_by(vd17_id)
  ) %>%
  mutate(db_status = if_else(is.na(db_status) & manual_status_override != "Not member linked", "Filtered out as variant or unified multi-part record", coalesce(db_status, manual_status_override, auto_status))) %>%
  add_metadata_fields() %>%
  collect() %>%
  ...
  prepare_for_gsheets() %>%
  arrange(desc(manual_status_override), desc(db_status), method, vd17_id) %>%
  relocate(manual_status_override, db_status, auto_status, notes, method) %>%
  sheet_write(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "VD17 <-> FBS master record data")
```

```{r}
manual_filtering_additions <-
  read_sheet("10jsxSQ_G8Q0BUjXR3KUZzBkXh829hiKp4A7uLWB7X5c", col_types = "c") %>%
  ...
  union_all(
    read_sheet("1J0Dfw8s_f9GdAJa9gQjJ6r9JFiR9nr0jKB11DmyIgjc", col_types = "c") %>%
      select(vd17_id, manual_status_override = manual_status_override...14, notes = notes...15) %>%
  ...
  union_all(
    read_sheet("12ArIbtnDmzflZA2DEBJ1Ksc4FL6LoAyiRU5_opa4vTo", col_types = "c") %>%
      mutate(notes = str_c(ja_notes, tl_notes, sep = "|")) %>%
      select(vd17_id, manual_status_override = society_related, notes) %>%
      mutate(override_source = "Genres double check")
  ) %>%
  union_all(
    read_sheet("1LTnnKN6Qvp5sr3PheqqhPVU6SrLKUPNmk3C9vTX2C4c", col_types = "c") %>%
      select(vd17_id, manual_status_override = manual_override, notes = notes...15) %>%
      mutate(override_source = "No/only neutral genres")
  ) %>%
  union_all(
    read_sheet("1YOpzA-kTMJuneP9FsFjZSXlx4CcrSNv6EyLAIYzNyEs", col_types = "c") %>%
      select(vd17_id, manual_status_override = society_related, notes) %>%
      mutate(override_source = "Occasional publications in German")
  ) %>%
  union_all(
    read_sheet("1vzppY79zpys_o5aftut5KdhGqzOplInJj-An4rDVlkY", col_types = "c") %>%
      select(vd17_id, manual_status_override, notes = notes...15) %>%
      mutate(override_source = "Missing occasional publications")
  ) %>%
  union_all(
    read_sheet("1o709k0ZlSBYll9J78vZlVnrk1BANSHWII1yPpXe4xE4", col_types = "c") %>%
      select(vd17_id, manual_status_override = manual_override, notes = notes...15) %>%
...
old_master_records %>%
  ...
  prepare_for_gsheets() %>%
  arrange(desc(manual_status_override), desc(auto_status), method, vd17_id) %>%
  relocate(manual_status_override, auto_status, notes, method) %>%
  sheet_write(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "VD17 <-> FBS master record data")
```

```{r mark_manual_decisions}
old_master_records %>%
  left_join(manual_filtering_addition %>% filter(is.na(manual_status_override)) %>% distinct(vd17_id) %>% mutate(checked = T)) %>%
  replace_na(list(checked = F)) %>%
  mutate(manual_status_override = if_else(checked, coalesce(manual_status_override, auto_status), manual_status_override)) %>%
  select(-checked) %>%
  sheet_write(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "VD17 <-> FBS master record data")
```
...
```{r}
genre_cats <- read_sheet(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "Genre categorisations") %>%
  select(-c(VD17, `Society-related`)) %>%
  rename(full_genre = genre)
```
  ...
  union_all(
    vd17_genres_a %>%
      inner_join(fbs_records_a %>%
        inner_join(vd17_id_a)) %>%
      count(full_genre, set)
  ) %>%
  pivot_wider(names_from = set, values_from = n) %>%
  collect() %>%
  full_join(genre_cats) %>%
  relocate(group_1, group_2, group_3, full_genre) %>%
  arrange(group_1, group_2, group_3) %>%
  write_sheet(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "Genre categorisations")
```

```{r}
vd17_genre_categorisation_gs <- read_sheet(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "Genre categorisations")
vd17_genre_categorisation_a <- vd17_genre_categorisation_gs %>%
  select(group_1:definition) %>%
  copy_to_a(con, "vd17_genre_categorisation_a", temporary = FALSE, overwrite = TRUE, unique_indexes = list(c("full_genre")))

vd17_genre_categorisation_c <- vd17_genre_categorisation_a %>%
  compute_c("vd17_genre_categorisation_c", temporary = FALSE, overwrite = TRUE)
```
