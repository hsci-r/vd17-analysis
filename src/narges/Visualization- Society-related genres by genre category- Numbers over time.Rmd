...
```{r}
fbs_records <- fbs_records_a %>%
  inner_join(vd17_id_a) %>%
  inner_join(fbs_links_a, by = ("record_number")) %>%
...
fbs_records_10 <- fbs_records_german %>%
  ...
  with(aggregate(member_number ~ normalized_year, FUN = function(x) {
    length(unique(x))
  })) %>%
...
vd17_society_genres <- vd17_genre_categorisation_a %>%
  ...
  collect() %>%
  select(record_number, group_2, genre)
```
...
core_society_genre <- core_society %>%
  ...
  mutate(genre_category = group_2) %>%
  select(record_number, normalized_year, vd17_id, genre_category, genre) %>%
  distinct()
...
core_society_frequency <- core_society_genre %>%
  select(vd17_id, record_number, normalized_year, genre, genre_category) %>%
  distinct(vd17_id, record_number, normalized_year, genre, genre_category) %>%
  group_by(normalized_year) %>%
  count(genre_category) %>%
  arrange(desc(n)) %>%
  distinct()
