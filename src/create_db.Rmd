...
vd17_titles_a <- vd17_a %>%
  filter(field_code == "036C") %>%
  pivot_wider(id_cols = c(record_number, field_number, field_code), values_from = value, names_from = subfield_code) %>%
...
vd17_combined_titles_a <- vd17_titles_a %>%
  group_by(record_number) %>%
  summarise(combined_title = str_flatten(combined_title, collapse = " / "), .groups = "drop") %>%
  compute_a(name = dbplyr::in_schema("vd17", "vd17_combined_titles_a"), temporary = FALSE, overwrite = TRUE, unique_indexes = list(c("record_number")))

vd17_combined_titles_c <- vd17_combined_titles_a %>%
  compute_c(name = dbplyr::in_schema("vd17", "vd17_combined_titles_c"), temporary = FALSE, overwrite = TRUE)
...
vd17_genre_categorisation_a <- read_sheet(ss = "1rXnOSB7P6vhJ5kTPu33upwZM__b0vx602vzBN63FaBw", sheet = "Genre categorisations") %>%
  ...
  copy_to_a(con, name = "vd17_genre_categorisation_a", unique_indexes = list(c("full_genre")), temporary = FALSE, overwrite = TRUE)

vd17_genre_categorisation_c <- vd17_genre_categorisation_a %>%
  compute_c(name = "vd17_genre_categorisation_c", temporary = FALSE, overwrite = TRUE)
```
...
```{r create_fbs_tables}
library(jsonlite)
fbs_metadata_json <- read_json(here("data/input/fr_mitglieder.json"), simplifyVector = T) %>%
  tibble() %>%
  mutate(gnd = if_else(gnd == "", NA_character_, gnd)) %>%
  rename(GND = gnd, member_number = nr, family_name = name) %>%
...
fbs_metadata_a <- fbs_metadata_json %>%
  left_join(fbs_metadata_gs %>% rename(`Old GND 2` = GND), join_by(member_number)) %>%
...
fbs_gnds_a <- fbs_metadata_a %>%
  ...
  union(
    fbs_metadata_a %>%
      select(member_number, GND = `Old GND 2`)
  ) %>%
  filter(!is.na(GND)) %>%
...
translations_into_german <- vd17_normalized_langs_a %>%
  filter(publication_language == "ger") %>%
  distinct(record_number) %>%
  inner_join(vd17_normalized_langs_a %>%
    filter(!is.na(original_language) | !is.na(intermediary_language)) %>%
    distinct(record_number)) %>%
  inner_join(vd17_id_a, join_by(record_number))

fbs_records_a <- fbs_records_a %>%
  left_join(translations_into_german) %>%
  mutate(set = if_else(set != "Member linked" & !is.na(record_number), "Member substantive role and translation into German", set)) %>%
  select(-record_number) %>%
  compute_a()

fbs_records_a <- fbs_records_a %>%
  union_all(fbs_records_a %>%
    filter(set == "Member substantive role and translation into German") %>%
    mutate(set = "Member substantive role and society purpose related")) %>%
  union_all(fbs_records_a %>%
    filter(set %in% c("Member substantive role and society purpose related", "Member substantive role and translation into German")) %>%
